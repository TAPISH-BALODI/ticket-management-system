...
const [rows, setRows] = useState([]);
    ...
    function onPayClickListener( item){
      const easeBuzzPayload = {
        ...
        axiosInstance.post("/easebuzz/payment_link/create", easeBuzzPayload)
        .then((res) => {
 ...
 React.useEffect(() => {
   console.log(location?.state?.user?._id)
        ...
        axiosInstance.get("/mypayments/fetch/" + location?.state?.user?._id)
      .then((res)=>{
        ...
        if(res.status === 200){
            ...
            res?.data?.res.forEach(element => {
              ...
              axiosInstance.post("/easebuzz/transaction/status", txn)
              .then((res) => {
                // console.log(res)
              })
              .catch((e) => {
...
<rect x="0.461426" y="0.772095" width="4.17139" height="21.7188" rx="2.08569" fill="#D32028"/>
        ...
        {rows?.map((item)=>{
          if(item?.status==="Pending" || item?.status==="preinitiated" || item?.status==="Collect")
          {
               return(
  ...
  Program/Class
          ...
          Payable Fee
              ...
              <Grid item xs={12}  lg={2} >
                  <Button onClick={()=>onPayClickListener(item)}variant='contained' color='primary' className='paynowbtn'  >
